...
public class ConvolutionReverbUploadIR : MonoBehaviour {
     ...
     #endif
    private static extern bool ConvolutionReverb_UploadSample(int index, float[] data, int numsamples, int numchannels,
        int samplerate, [MarshalAs(UnmanagedType.LPStr)] string name);
    ...
    void UploadChangedClips() {
        ...
        foreach (var s in impulse) {
            ...
            if (s != null && s.loadState == AudioDataLoadState.Loaded && !uploaded[currindex]) {
                ...
                s.GetData(data, 0);
                ConvolutionReverb_UploadSample(currindex, data, data.Length / s.channels, s.channels, s.frequency,
                    s.name);
